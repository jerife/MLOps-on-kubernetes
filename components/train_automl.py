...
@partial(
    ...
    packages_to_install=["tqdm==4.64.0", "numpy==1.21.6", "mne==1.1.0", "dill==0.3.4", "scikit_learn==1.0.2", "mlflow==1.21.0"],
)
def train_with_wandb(
    ...
    import dill
    with open(train_x_path, mode="wb") as file_reader:
        train_x = dill.load(file_reader)
    with open(train_y_path, mode="wb") as file_reader:
        train_y = dill.load(file_reader)
    with open(test_x_path, mode="wb") as file_reader:
        test_x = dill.load(file_reader)
    with open(test_y_path, mode="wb") as file_reader:
    ...
    input_example = train_x.sample(1)
